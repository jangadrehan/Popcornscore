...
$ref_id = $_GET['id'];
$ref_type = 'movie'; // adjust dynamically if needed

$sql = "SELECT * FROM reviews WHERE user_id = ? AND ref_id = ? AND ref_type = ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param("iis", $user_id, $ref_id, $ref_type);
$stmt->execute();
$result = $stmt->get_result();
$review = $result->fetch_assoc();  // ✅ This makes $review an array
...
if (isset($_POST['update_review'])) {
    ...
    if ($stmt->execute()) {
        header("Location: details.php?id=" . $ref_id); // refresh with updated review
        exit;
    } else {
...
// Fetch cast
$cast_stmt = $conn->prepare("SELECT * FROM cast WHERE ref_id = ? AND ref_type = 'movie'");
...
<div class="carousel-item active">
    ...
    <div class="carousel-caption d-none d-md-block">
        ...
        <a href="<?= htmlspecialchars($movie['wiki']) ?>"
...
<!-- Main Carousel Section End -->
 ...
 <div class="container mt-5 wow fadeInDown" data-wow-delay="0.5s" >
                 
    <!-- Movie Info -->
    <div class="row">
        <div class="col-md-4">
            <?php if (!empty($movie['photo'])): ?>
                <img src="admin/<?= $movie['photo']; ?>" class="img-fluid rounded">
            <?php endif; ?>
        </div>
         <div class="col-md-8">
            <h2><?= htmlspecialchars($movie['moviename']) ?></h2>
            <h5><strong>Directed by:</strong> <?= htmlspecialchars($movie['director']) ?></h5>
            <h5><strong>Produced by:</strong> <?= htmlspecialchars($movie['producer']) ?></h5>
            <h6><?= nl2br(htmlspecialchars($movie['details'])) ?></h6>
        </div>
    </div>
...
<hr>
    <!-- Cast Section -->
    <h3>Cast</h3>
    <div class="row section wow fadeInDown" data-wow-delay="0.4s">
...
$movie_id = $_GET['id']; // movie or tvshow ID
...
$query = "SELECT rating FROM reviews WHERE ref_id = '$movie_id'";
$result = mysqli_query($conn, $query);
...
<?php if (!$user_rating): ?>
    ...
    <form action="auth_procces/addreview_process.php?id=<?=$ref_id?>&type=movie" method="POST" id="rating-form">
        ...
        <?php while ($row = $result->fetch_assoc()): ?>
            <div class="comment-item" 
                 style="padding:10px; margin-bottom:8px; border-radius:8px;
                        background-color: <?= ($row['user_id'] == $current_user_id) ? '#17b1f8ff' : '#f9f9f9' ?>;">
...
<style>
 .review-display {
    ...
    border-left: 4px solid #1973f1ff;
    border-radius: 6px;
